...
class OrderExecutionBit(object):
    ...
    OrderExecutionBit -> "bit" , bit 仅仅持有订单信息, 并不决定订单是否全部或者部分交易。
    ...
    def log(self):
        strLog = '\ndt: {dt},\nsize: {size},\nprice: {price},\nclosed: {closed},\n'\
                 ...
                 'comm: {comm},\nclosedcomm: {closedcomm},\nopenedcomm： {openedcomm},\npnl: {pnl},\npsize: {psize},\npprice: {pprice}\n'.format(
                     ...
                     opened=self.opened,
                     value=self.value,
                     closedvalue=self.closedvalue,
                     openedvalue=self.openedvalue,
                     comm=self.comm,
                     closedcomm=self.closedcomm,
                     openedcomm=self.openedcomm,
                     pnl=self.pnl,
                     psize=self.psize,
                     pprice=self.pprice
                     )

        self.__log.log(strLog)
...
class OrderBase(with_metaclass(MetaParams, object)):
    ...
    # 订单状态
    # Created: 订单创建
    # Submitted: 订单提交经纪人, 待确认
    # Accepted: 订单被经纪人接受
    # Partial: 订单部分执行
    # Completed: 订单完全执行
    # Canceled/Cancelled: 订单被用户取消
    # Expired: 订单过期
    # Margin： 保证金不足，没有足够的现金执行订单
    # Rejected： 经纪人拒绝订单
    Status = [
        'Created', 'Submitted', 'Accepted', 'Partial', 'Completed',
